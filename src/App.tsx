...
type View = 'inicio' | 'explorar' | 'pedidos' | 'perfil'

const promos = [
  { title: '¡Primer pedido GRATIS!', sub: 'Usa el código: SIERRA1', bg: 'from-[#5bc827] to-[#3d8c18]', text: 'text-[#0d1a0f]', emoji: '🎉' },
  { title: 'Envío gratis los martes', sub: 'En restaurantes seleccionados', bg: 'from-[#1a3320] to-[#0d1a0f]', text: 'text-[#5bc827]', emoji: '🛵' },
  { title: '2x1 en combos hoy', sub: 'Solo hasta las 11pm', bg: 'from-[#142a17] to-[#1a3320]', text: 'text-white', emoji: '🍔' },
]
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0d1a0f] text-white">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-[#0d1a0f]/95 backdrop-blur-sm border-b border-[#2a4830]">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {!selectedRestaurant && (
            <button className="flex items-center gap-1 text-xs text-[#7aaa70] hover:text-[#5bc827] transition-colors truncate max-w-[160px] sm:max-w-xs">
          ...
          <div className="flex-1 relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#7aaa70]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input type="text" placeholder="Busca restaurantes o platillos..."
              ...
              className="w-full bg-[#142a17] border border-[#2a4830] rounded-full py-2 pl-9 pr-4 text-sm text-white placeholder-[#7aaa70] focus:outline-none focus:border-[#5bc827] transition-colors" />
          </div>

          <button onClick={() => setCartOpen(true)} className="relative shrink-0 bg-[#5bc827] hover:bg-[#7ed944] text-[#0d1a0f] rounded-full p-2 transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-white text-[#0d1a0f] text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{cartCount}</span>
            )}
          </button>
        </div>
      </header>
      ...
      {selectedRestaurant ? (
        ...
        <main className="max-w-5xl mx-auto">
          ...
          {view === 'perfil' && <Perfil role={role} onLogout={() => setRole(null)} />}
        </main>
      )}
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-[#0d1a0f]/95 backdrop-blur-sm border-t border-[#2a4830] flex justify-around py-2 z-50">
          {navItems.map(item => (
            <button key={item.view} onClick={() => setView(item.view)}
              className={`flex flex-col items-center gap-0.5 px-4 py-1 transition-colors ${view === item.view ? 'text-[#5bc827]' : 'text-[#7aaa70] hover:text-[#a8d89a]'}`}>
      ...
      <CartDrawer
        ...
        onRemove={removeItem}
        onCheckout={() => { setCartOpen(false); setView('pedidos') }}
...
function HomeView({
  ...
  return (
    <div className="px-4 pb-24">
      {/* Hero */}
      <section className="mt-5 rounded-2xl overflow-hidden relative bg-[#142a17] border border-[#2a4830]">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0d1a0f] via-[#0d1a0f]/70 to-transparent" />
        </div>
        <div className="relative px-6 py-8 sm:py-12 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#a8d89a] text-sm mb-5 max-w-xs">Restaurantes, súper y farmacia. Entrega rápida en tu zona.</p>
            <button onClick={() => onSelectRestaurant(allRestaurants[0])}
              className="bg-[#5bc827] hover:bg-[#7ed944] text-[#0d1a0f] font-bold text-sm px-6 py-2.5 rounded-full transition-all hover:scale-105 active:scale-95">
              Pedir ahora
            </button>
          </div>
          <div className="hidden sm:flex flex-col gap-3 text-right">
            {[{ n: '15 min', l: 'Entrega rápida' }, { n: '+200', l: 'Restaurantes' }, { n: '24/7', l: 'Disponible' }].map(s => (
              <div key={s.n}>
                ...
                <div className="text-xs text-[#7aaa70]">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-6">
        <div className="flex gap-3 overflow-x-auto pb-1">
          {promos.map((p, i) => (
            <div key={i} className={`flex-shrink-0 rounded-xl bg-gradient-to-br ${p.bg} border border-[#2a4830] p-4 min-w-[200px] flex items-center gap-3 cursor-pointer hover:scale-[1.02] transition-transform`}>
              <span className="text-3xl">{p.emoji}</span>
              <div>
                <p className={`font-bold text-sm leading-tight ${p.text}`}>{p.title}</p>
                <p className="text-[#7aaa70] text-xs mt-0.5">{p.sub}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {categories.map(cat => (
            <button key={cat.label} onClick={() => setActiveCategory(cat.label)}
              className={`flex-shrink-0 flex flex-col items-center gap-1.5 px-4 py-3 rounded-xl border transition-all ${activeCategory === cat.label ? 'bg-[#5bc827] border-[#5bc827] text-[#0d1a0f]' : 'bg-[#142a17] border-[#2a4830] text-[#a8d89a] hover:border-[#5bc827] hover:text-white'}`}>
...
function RestaurantCard({ r, onClick }: { r: Restaurant; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} className="bg-[#142a17] border border-[#2a4830] rounded-2xl overflow-hidden group cursor-pointer hover:border-[#5bc827]/50 transition-all hover:shadow-lg hover:shadow-[#5bc827]/10">
      <div className="relative h-40 overflow-hidden bg-[#1a3320]">
        ...
        {r.badge && (
          <span className="absolute top-2 left-2 bg-[#5bc827] text-[#0d1a0f] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">{r.badge}</span>
        )}
        {r.promo && (
          <span className="absolute bottom-2 left-2 bg-[#0d1a0f]/80 text-[#5bc827] text-[10px] font-semibold px-2 py-0.5 rounded-full border border-[#5bc827]/40">🏷 {r.promo}</span>
        )}
        {!r.isOpen && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
            <span className="bg-[#0d1a0f]/80 text-[#7aaa70] text-xs font-bold px-3 py-1 rounded-full border border-[#2a4830]">Cerrado</span>
          </div>
        )}
        <button onClick={e => { e.stopPropagation(); setLiked(l => !l) }}
          className="absolute top-2 right-2 bg-[#0d1a0f]/60 rounded-full p-1.5 hover:bg-[#0d1a0f]/80 transition-colors">
      ...
      <div className="p-3">
        ...
        <p className="text-[#7aaa70] text-xs mb-2">{r.category}</p>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-[10px] text-[#7aaa70]">
            <span>⏱ {r.time}</span>
