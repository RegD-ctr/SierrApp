...
const roles = [
  ...
  { id: 'repartidor' as Role, label: 'Repartidor', desc: 'Gestiona tus entregas', icon: '🏍️', border: 'border-[#7ed944]', text: 'text-[#7ed944]', gradient: 'from-[#2a4830] to-[#142a17]' },
]
...
export default function Login({ onLogin }: Props) {
  ...
  const LogoHeader = () => (
    <div className="flex flex-col items-center mb-8">
      ...
      <p className="text-[#7aaa70] text-xs tracking-widest uppercase mt-1">Todo lo que necesitas, a un toque.</p>
    </div>
  )

  const BackBtn = ({ to, label = 'Volver' }: { to: Screen; label?: string }) => (
    <button onClick={() => setScreen(to)} className="flex items-center gap-1 text-[#7aaa70] hover:text-[#5bc827] text-sm mb-5 transition-colors">
  ...
  if (screen === 'roleSelect') return (
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      {BG}
      <div className="relative w-full max-w-sm">
        <LogoHeader />
        <p className="text-center text-[#a8d89a] text-sm mb-5">¿Cómo quieres ingresar?</p>
        <div className="space-y-3 mb-5">
          {roles.map(role => (
            <button key={role.id} onClick={() => { setSelectedRole(role.id); setScreen('login') }}
              className={`w-full flex items-center gap-4 p-4 rounded-2xl border ${role.border} bg-[#142a17] hover:bg-[#1a3320] transition-all hover:scale-[1.02] active:scale-[0.98] text-left`}>
              <span className="text-3xl">{role.icon}</span>
              <div className="flex-1">
                <p className={`font-bold text-base ${role.text}`}>{role.label}</p>
                <p className="text-[#7aaa70] text-xs">{role.desc}</p>
              </div>
        ...
        <p className="text-center text-[#7aaa70] text-sm">
          ¿No tienes cuenta?{' '}
  ...
  if (screen === 'login' && activeRole) return (
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#142a17] border ${activeRole.border} mb-5`}>
        ...
        <p className="text-[#7aaa70] text-xs mb-6">
          ¿No tienes cuenta?{' '}
        ...
        <form onSubmit={e => { e.preventDefault(); onLogin(selectedRole!) }} className="space-y-3">
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Correo electrónico</label>
            <input type="email" placeholder="ejemplo@correo.com" value={email} onChange={e => setEmail(e.target.value)} required
              className="w-full bg-[#142a17] border border-[#2a4830] focus:border-[#5bc827] rounded-xl px-4 py-3 text-sm text-white placeholder-[#7aaa70] outline-none transition-colors" />
          </div>
          <div className="relative">
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Contraseña</label>
            <input type={showPass ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required
              className="w-full bg-[#142a17] border border-[#2a4830] focus:border-[#5bc827] rounded-xl px-4 py-3 text-sm text-white placeholder-[#7aaa70] outline-none transition-colors pr-10" />
            <button type="button" onClick={() => setShowPass(s => !s)} className="absolute right-3 bottom-3 text-[#7aaa70]">{showPass ? '🙈' : '👁️'}</button>
          </div>
  ...
  if (screen === 'register_roleSelect') return (
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <p className="text-[#7aaa70] text-sm mb-6">¿Qué tipo de cuenta deseas crear?</p>
          ...
          {[
            ...
            <button key={opt.id} onClick={() => setScreen(opt.screen)}
              ...
              <div className="flex-1">
                ...
                <p className="text-[#7aaa70] text-xs">{opt.desc}</p>
              </div>
  ...
  if (screen === 'register_usuario') return (
    <div className="min-h-screen bg-[#0d1a0f] px-4 py-6 relative overflow-hidden">
      {BG}
      <div className="relative w-full max-w-sm mx-auto">
        <BackBtn to="register_roleSelect" />
        <div className="flex items-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-full bg-[#5bc827] flex items-center justify-center text-[#0d1a0f] font-bold text-sm">👤</div>
        ...
        <form onSubmit={e => { e.preventDefault(); onLogin('usuario') }} className="space-y-3">
          ...
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Detalles para encontrar tu dirección</label>
          ...
          {/* Map placeholder */}
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Ubicación en mapa</label>
            <div className="relative h-36 rounded-xl overflow-hidden border border-[#2a4830] bg-[#0a1a0c]">
          ...
          <button type="submit" className="w-full py-3.5 rounded-xl bg-[#5bc827] hover:bg-[#7ed944] text-[#0d1a0f] font-bold text-sm transition-all hover:scale-[1.02] mt-2 shadow-lg shadow-[#5bc827]/20">
            Crear cuenta
          </button>
        </form>
      </div>
  ...
  if (screen === 'register_local') return (
    <div className="min-h-screen bg-[#0d1a0f] px-4 py-6 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm mx-auto">
        <BackBtn to="register_roleSelect" />
        <div className="flex items-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-full bg-[#4dbd5a] flex items-center justify-center text-[#0d1a0f] font-bold text-sm">🏪</div>
        ...
        <form onSubmit={e => { e.preventDefault(); setScreen('register_local_platillos') }} className="space-y-3">
          ...
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Logo del restaurante</label>
            <div className="h-24 border-2 border-dashed border-[#2a4830] hover:border-[#5bc827] rounded-xl flex items-center justify-center cursor-pointer transition-colors">
              <div className="flex flex-col items-center gap-1 text-[#7aaa70]"><span className="text-2xl">🖼️</span><span className="text-xs">Subir logo</span></div>
            </div>
          </div>
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Imagen de portada</label>
            <div className="h-24 border-2 border-dashed border-[#2a4830] hover:border-[#5bc827] rounded-xl flex items-center justify-center cursor-pointer transition-colors">
              <div className="flex flex-col items-center gap-1 text-[#7aaa70]"><span className="text-2xl">📷</span><span className="text-xs">Subir portada</span></div>
            </div>
          </div>

          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Ubicación del restaurante</label>
            <div className="relative h-36 rounded-xl overflow-hidden border border-[#2a4830] bg-[#0a1a0c]">
              <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(42,72,48,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(42,72,48,0.4) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
                <div className="relative">
  ...
  if (screen === 'register_local_platillos') return (
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm text-center">
        ...
        <p className="text-[#7aaa70] text-sm mb-6">Ahora agrega tus primeros platillos para que los clientes puedan ordenar.</p>
        ...
        <button onClick={() => onLogin('local')} className="text-[#7aaa70] text-xs mt-3 hover:text-white transition-colors">
          Saltar por ahora →
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="flex items-center gap-2 mb-5">
          <div className="w-8 h-8 rounded-full bg-[#7ed944] flex items-center justify-center text-[#0d1a0f] font-bold text-sm">🏍️</div>
        ...
        <form onSubmit={e => { e.preventDefault(); setScreen('register_repartidor_result') }} className="space-y-4">
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Nombre completo</label>
            <input value={rNombre} onChange={e => setRNombre(e.target.value)} required placeholder="Juan Pérez"
              className="w-full bg-[#142a17] border border-[#2a4830] focus:border-[#7ed944] rounded-xl px-4 py-3 text-sm text-white placeholder-[#7aaa70] outline-none transition-colors" />
          </div>
          <div>
            <label className="text-[#a8d89a] text-xs font-semibold block mb-1">Fotografía de perfil</label>
            <div onClick={() => fotoRef.current?.click()}
              className="h-32 border-2 border-dashed border-[#2a4830] hover:border-[#7ed944] rounded-xl flex items-center justify-center cursor-pointer overflow-hidden transition-colors">
              {rFoto ? (
                <img src={rFoto} alt="preview" className="w-full h-full object-cover" />
              ) : (
                <div className="flex flex-col items-center gap-1 text-[#7aaa70]"><span className="text-3xl">📷</span><span className="text-xs">Subir foto</span></div>
              )}
            </div>
            <input ref={fotoRef} type="file" accept="image/*" onChange={handleFoto} className="hidden" />
          </div>
          <button type="submit" className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#2a4830] to-[#142a17] border border-[#7ed944] text-[#7ed944] font-bold text-sm transition-all hover:scale-[1.02]">
  ...
  if (screen === 'register_repartidor_result') return (
    <div className="min-h-screen bg-[#0d1a0f] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      {BG}
      <div className="relative w-full max-w-sm text-center">
        <div className="w-20 h-20 rounded-full mx-auto mb-4 overflow-hidden border-2 border-[#7ed944] bg-[#142a17] flex items-center justify-center">
        ...
        <p className="text-[#7aaa70] text-sm mt-1 mb-6">Bienvenido a Sierra App, {rNombre || 'Repartidor'}</p>

        <div className="bg-[#142a17] border border-[#7ed944]/50 rounded-2xl p-5 mb-6 text-left">
          <div className="mb-3 pb-3 border-b border-[#2a4830]">
            <p className="text-[#7aaa70] text-[10px] uppercase tracking-widest">Nombre</p>
            <p className="text-white font-bold text-base">{rNombre || 'Juan Pérez'}</p>
          </div>
          <div className="mb-3 pb-3 border-b border-[#2a4830]">
            <p className="text-[#7aaa70] text-[10px] uppercase tracking-widest">Matrícula</p>
            <p className="text-[#7ed944] font-bold text-2xl tracking-widest" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>{rMatricula}</p>
            <p className="text-[#7aaa70] text-[10px] mt-0.5">Generada automáticamente · No la compartas</p>
          </div>
          {rFoto && (
            <div>
              <p className="text-[#7aaa70] text-[10px] uppercase tracking-widest mb-1">Foto</p>
        ...
        <button onClick={() => onLogin('repartidor')}
          className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#2a4830] to-[#142a17] border border-[#7ed944] text-[#7ed944] font-bold text-sm transition-all hover:scale-[1.02] shadow-lg">
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[#5bc827] text-xs font-bold uppercase tracking-widest pt-2 border-t border-[#2a4830]">{children}</p>
  )
}
...
function Field({ label, value, onChange, placeholder, type = 'text', error = '' }: {
  ...
  return (
    <div>
      <label className="text-[#a8d89a] text-xs font-semibold block mb-1">{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className={`w-full bg-[#142a17] border ${error ? 'border-red-600' : 'border-[#2a4830] focus:border-[#5bc827]'} rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#7aaa70] outline-none transition-colors`} />
