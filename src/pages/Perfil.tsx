...
const menuItems = [
  { icon: '📍', label: 'Mis direcciones', sub: 'Calle Pino #24, Sierra Norte' },
  { icon: '💳', label: 'Métodos de pago', sub: '•••• 4821 · Efectivo' },
  { icon: '🎁', label: 'Promociones y cupones', sub: 'SIERRA1 disponible' },
  { icon: '⭐', label: 'Favoritos', sub: '3 restaurantes guardados' },
  { icon: '🔔', label: 'Notificaciones', sub: 'Activadas' },
  { icon: '🔒', label: 'Privacidad y seguridad', sub: null },
  { icon: '❓', label: 'Ayuda y soporte', sub: null },
...
export default function Perfil({ role, onLogout }: PerfilProps) {
  const roleInfo = roleLabels[role]

  return (
    <div className="min-h-screen bg-[#0d1a0f] pb-24">
      {/* Header hero */}
      <div className="relative bg-gradient-to-b from-[#142a17] to-[#0d1a0f] px-4 pt-8 pb-6 border-b border-[#2a4830]">
        <h1
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            <button className="absolute bottom-0 right-0 bg-[#5bc827] rounded-full w-5 h-5 flex items-center justify-center text-[8px] text-[#0d1a0f]">
              ✏️
            </button>
          </div>
          <div className="flex-1">
            <h2 className="text-white font-bold text-lg leading-tight">Juan Sierra</h2>
            <p className="text-[#7aaa70] text-xs">juan.sierra@email.com</p>
            <div className={`inline-flex items-center gap-1.5 mt-1.5 px-2.5 py-1 rounded-full bg-[#0d1a0f] border border-[#2a4830]`}>
              <span className="text-xs">{roleInfo.icon}</span>
              <span className={`text-[10px] font-bold ${roleInfo.color}`}>{roleInfo.label}</span>
            </div>
          </div>
          <button className="text-[#7aaa70] hover:text-[#5bc827] transition-colors">
        ...
        {/* Stats */}
        <div className="flex mt-5 bg-[#0d1a0f] rounded-2xl border border-[#2a4830] overflow-hidden">
          {stats.map((s, i) => (
            <div key={s.l} className={`flex-1 py-3 text-center ${i < stats.length - 1 ? 'border-r border-[#2a4830]' : ''}`}>
              <p
      ...
      <div className="px-4 pt-5 space-y-1">
        {menuItems.map(item => (
          <button
            key={item.label}
            className="w-full flex items-center gap-3 px-3 py-3.5 rounded-xl hover:bg-[#142a17] transition-colors text-left group"
          >
            <span className="text-xl w-7 text-center">{item.icon}</span>
            <div className="flex-1">
              <p className="text-sm text-white font-medium group-hover:text-[#5bc827] transition-colors">{item.label}</p>
              {item.sub && <p className="text-[10px] text-[#7aaa70] mt-0.5">{item.sub}</p>}
            </div>
            <svg className="w-4 h-4 text-[#2a4830] group-hover:text-[#5bc827] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
