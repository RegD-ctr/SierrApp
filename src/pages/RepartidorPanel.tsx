...
type RepView = 'mapa' | 'ordenes' | 'activa' | 'historial' | 'perfil'
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#7aaa70] text-[10px]">Panel de Repartidor</p>
            </div>
          </div>
          <button
            onClick={() => setIsOnline(v => !v)}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold transition-all ${
              isOnline ? 'border-[#5bc827]/50 text-[#5bc827] bg-[#5bc827]/10' : 'border-[#2a4830] text-[#7aaa70]'
            }`}
          >
            <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-[#5bc827] animate-pulse' : 'bg-[#7aaa70]'}`} />
      ...
      <main className="max-w-5xl mx-auto pb-28">
            ...
            <div className="relative h-[55vh] bg-[#0a1a0c] overflow-hidden">
              ...
              <div className="absolute" style={{ left: '28%', top: '33%', transform: 'translate(-50%,-100%)' }}>
                <div className="flex flex-col items-center">
                  <div className="bg-[#142a17] border-2 border-[#5bc827] rounded-xl px-2 py-1 text-[10px] font-bold text-[#5bc827] whitespace-nowrap mb-1">
                    🏪 Sierra Burger
                  </div>
              ...
              <div className="absolute" style={{ left: '74%', top: '28%', transform: 'translate(-50%,-100%)' }}>
                <div className="flex flex-col items-center">
                  <div className="bg-[#142a17] border-2 border-[#7ed944] rounded-xl px-2 py-1 text-[10px] font-bold text-[#7ed944] whitespace-nowrap mb-1">
                    🏠 Juan Sierra
                  </div>
              ...
              {/* Map overlay bottom */}
              <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#0d1a0f] to-transparent" />
            </div>
              ...
              {isOnline ? (
                orders.length > 0 ? (
                  <div className="space-y-3">
                    ...
                    {orders.map(o => (
                      <div
                        ...
                        className="bg-[#142a17] border border-[#2a4830] hover:border-[#5bc827]/50 rounded-2xl p-4 flex items-center justify-between cursor-pointer transition-all"
                      >
                        <div>
                          <div className="flex items-center gap-2">
                          ...
                          <p className="text-[#7aaa70] text-xs">{o.hora} · {o.total}</p>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-[#7aaa70] text-xs">Mantente en línea para recibir pedidos</p>
                  </div>
                )
              ) : (
                <div className="flex flex-col items-center py-8 text-center">
                  ...
                  <p className="text-[#7aaa70] text-sm mt-1">Conéctate para recibir órdenes</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-4 pt-5">
            ...
            {orders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-[#7aaa70] text-sm mt-1">Mantente en línea para recibir pedidos</p>
              </div>
            ) : (
              <div className="space-y-3">
                {orders.map(o => (
                  <div
                    key={o.id}
                    onClick={() => { setActiveOrderId(o.id); setView('activa') }}
                    className="bg-[#142a17] border border-[#2a4830] hover:border-[#5bc827]/50 rounded-2xl p-4 cursor-pointer transition-all"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p className="text-[#7aaa70] text-xs">📍 {o.localDir}</p>
                      </div>
                      <StatusBadge status={o.status} />
                    </div>
                    <div className="border-t border-[#2a4830] pt-2 mt-2 flex items-center justify-between">
                      <div>
                        <p className="text-[#7aaa70] text-xs">{o.productos.map(p => `${p.nombre} x${p.cantidad}`).join(', ')}</p>
                        <p className="text-[10px] text-[#7aaa70] mt-0.5">⏱ {o.hora}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[#5bc827] font-bold text-sm">{o.total}</p>
                        <p className="text-[10px] text-[#7aaa70]">{o.productos.reduce((a, p) => a + p.cantidad, 0)} productos</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="px-4 pt-5">
            {!activeOrder ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-[#7aaa70] text-sm mt-1 mb-4">Selecciona una orden de "Mis Órdenes"</p>
                <button onClick={() => setView('ordenes')} className="bg-[#5bc827] text-[#0d1a0f] font-bold px-5 py-2 rounded-full text-sm hover:bg-[#7ed944] transition-colors">
                  Ver órdenes
                </button>
              </div>
            ) : (
        ...
        {view === 'historial' && (
          <div className="px-4 pt-5">
            ...
            <div className="flex items-center justify-between mb-5">
              <p className="text-[#7aaa70] text-sm">{completedOrders.length} entregas realizadas</p>
              <div className="bg-[#142a17] border border-[#2a4830] rounded-xl px-3 py-1.5 text-xs text-[#5bc827] font-bold">
            ...
            {completedOrders.length === 0 ? (
              ...
              <div className="space-y-3">
                {completedOrders.map((o, i) => (
                  <div key={`${o.id}-${i}`} className="bg-[#142a17] border border-[#2a4830] rounded-2xl p-4">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-[10px] text-[#7aaa70]">{o.hora}</span>
        ...
        {view === 'perfil' && (
          <div className="px-4 pt-5">
            ...
            <div className="bg-[#142a17] border border-[#2a4830] rounded-2xl p-5 flex items-center gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-[#7aaa70] text-xs">carlos.mendez@sierraapp.mx</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-[10px] text-[#5bc827] font-bold">🛵 Repartidor</span>
                  <span className="text-[10px] text-[#7aaa70]">★ 4.9</span>
                  <span className="text-[10px] text-[#7aaa70]">142 entregas</span>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-3 bg-[#142a17] border border-[#2a4830] rounded-2xl overflow-hidden mb-5">
              {[{ v: '142', l: 'Entregas' }, { v: '$8,240', l: 'Ganancias' }, { v: '4.9★', l: 'Rating' }].map((s, i) => (
                <div key={s.l} className={`py-4 text-center ${i < 2 ? 'border-r border-[#2a4830]' : ''}`}>
                  <p className="text-[#5bc827] font-bold text-xl" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>{s.v}</p>
                  <p className="text-[#7aaa70] text-[10px]">{s.l}</p>
                </div>
              ))}
            </div>
            {[
              ...
              <button key={item.label} className="w-full flex items-center gap-3 px-3 py-3.5 rounded-xl hover:bg-[#142a17] transition-colors text-left">
                ...
                <div className="flex-1">
                  ...
                  {item.sub && <p className="text-[10px] text-[#7aaa70]">{item.sub}</p>}
                </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[#0d1a0f]/95 backdrop-blur-sm border-t border-[#2a4830] flex justify-around py-2 z-50">
        {navItems.map(item => (
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-3 py-1 transition-colors relative ${view === item.view ? 'text-[#5bc827]' : 'text-[#7aaa70]'}`}
          >
            <span className="text-xl">{item.icon}</span>
            <span className="text-[9px] font-medium">{item.label}</span>
            {view === item.view && <span className="w-1 h-1 rounded-full bg-[#5bc827] mt-0.5" />}
            {item.view === 'ordenes' && orders.length > 0 && (
              <span className="absolute top-0 right-2 bg-[#5bc827] text-[#0d1a0f] text-[8px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">
                {orders.length}
              </span>
            )}
          </button>
      ...
      {showConfirm === 'recibido' && activeOrder && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-[#0d1a0f] border border-[#2a4830] rounded-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-[#7aaa70] text-sm mb-5">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(null)} className="flex-1 py-3 rounded-xl border border-[#2a4830] text-[#7aaa70] text-sm font-semibold hover:text-white transition-colors">
                Cancelar
              </button>
              <button
                onClick={() => { advanceStatus(activeOrder.id); advanceStatus(activeOrder.id) }}
                className="flex-1 py-3 rounded-xl bg-[#5bc827] text-[#0d1a0f] font-bold text-sm hover:bg-[#7ed944] transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
                Confirmar ✅
              </button>
            </div>
      ...
      {showConfirm === 'entregado' && activeOrder && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-[#0d1a0f] border border-[#2a4830] rounded-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-[#7aaa70] text-sm mb-2">
...
function OrderDetail({ order, onAdvanceToRecibido, onAdvanceToEntregado }: {
  ...
  return (
    <div>
          ...
          return (
            <div key={s} className="flex items-center gap-1">
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full border text-[9px] font-bold transition-all whitespace-nowrap ${
                ...
                'border-[#2a4830] text-[#2a4830]'
              }`}>
                <span>{cfg.icon}</span>
              </div>
              {i < stateFlow.length - 2 && (
                <div className={`w-4 h-0.5 ${done ? 'bg-[#5bc827]' : 'bg-[#2a4830]'}`} />
              )}
            </div>
          )
        })}
      ...
      {/* Map mini */}
      <div className="relative h-44 rounded-2xl overflow-hidden bg-[#0a1a0c] mb-4 border border-[#2a4830]">
        <div className="absolute inset-0" style={{
        ...
        {!isEnCamino ? (
          <div className="absolute" style={{ left: '48%', top: '16%', transform: 'translate(-50%,-100%)' }}>
            <div className="flex flex-col items-center">
              <div className="bg-[#142a17] border border-[#5bc827] rounded-lg px-2 py-0.5 text-[9px] font-bold text-[#5bc827] whitespace-nowrap mb-1">🏪 {order.local}</div>
          ...
          <div className="absolute" style={{ left: '79%', top: '18%', transform: 'translate(-50%,-100%)' }}>
            <div className="flex flex-col items-center">
              <div className="bg-[#142a17] border border-[#7ed944] rounded-lg px-2 py-0.5 text-[9px] font-bold text-[#7ed944] whitespace-nowrap mb-1">🏠 {order.cliente}</div>
      ...
      {/* Info card */}
      <div className="bg-[#142a17] border border-[#2a4830] rounded-2xl p-4 mb-4">
        {!isEnCamino ? (
          <>
            <p className="text-[#7aaa70] text-[10px] uppercase tracking-widest font-semibold mb-1">Recoge en</p>
            <h3 className="text-white font-bold text-base">{order.local}</h3>
            <p className="text-[#7aaa70] text-xs mt-0.5 mb-3">📍 {order.localDir}</p>
          </>
        ) : (
          <>
            <p className="text-[#7aaa70] text-[10px] uppercase tracking-widest font-semibold mb-1">Entregar a</p>
            <h3 className="text-white font-bold text-base">{order.cliente}</h3>
            <p className="text-[#7aaa70] text-xs mt-0.5 mb-3">📍 {order.clienteDir}</p>
          </>
        )}
        <div className="border-t border-[#2a4830] pt-3">
          <p className="text-[#a8d89a] text-xs font-semibold mb-1">Productos:</p>
          {order.productos.map(p => (
            <div key={p.nombre} className="flex items-center justify-between text-xs">
              <span className="text-white">{p.nombre}</span>
              <span className="text-[#7aaa70]">x{p.cantidad}</span>
            </div>
          ))}
          {order.notas && (
      ...
      {!isEnCamino && order.status !== 'entregado' ? (
        <button
          ...
          className="w-full py-4 rounded-2xl bg-[#5bc827] hover:bg-[#7ed944] text-[#0d1a0f] font-bold text-base transition-all hover:scale-[1.01] active:scale-[0.99] shadow-lg shadow-[#5bc827]/20"
        >
